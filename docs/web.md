...
- 多选采集目标后触发一次采集，并设置每个目标采集条数
- Web 采集默认使用详情模式，后端会自动追加 `--details=true`
- 查看单次运行详情：本次成功入库几条、每条标题是什么
- 在内容页左侧查看采集列表，右侧查看完整详情、标签、链接、正文和原始 JSON
- 内容记录会标记详情状态、缺失字段和详情失败原因；采集使用本机非 headless 浏览器从搜索页逐条打开详情
- 批量笔记拆解
- 批量选题评分
- 批量生成草稿
...
- 标签取决于小红书详情页返回的数据；只采搜索卡片时可能没有标签，Web 采集已默认走详情采集以尽量获取标签和正文。
